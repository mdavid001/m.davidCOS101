...
fn main() {
    ...
    let no_of_siblings:usize = no_of_siblings.trim().parse().expect("Not a valid number");

    let mut names = vec![];
    let mut ages: Vec<u32> = vec![];
    let mut status = vec![""; no_of_siblings];
    let mut job = vec![""; no_of_siblings];
    let mut uni = vec!["".to_string(); no_of_siblings];
    let mut course = vec!["".to_string(); no_of_siblings];
    let mut offspring: Vec<u32> = vec![];
    let mut city = vec!["".to_string(); no_of_siblings];
    let mut waec = vec![""; no_of_siblings];
    let mut notwaec = vec![""; no_of_siblings];
    let mut school = vec!["".to_string(); no_of_siblings];
    let mut class = vec!["".to_string(); no_of_siblings];
    

    for number in 0..no_of_siblings {
        println!("\nENTER DETAILS FOR SIBLING {}
